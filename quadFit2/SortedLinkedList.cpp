...
template <class T>
SortedLinkedList<T>::SortedLinkedList(unsigned maxLen)
{
...
template <class T> void SortedLinkedList<T>::setMax(unsigned maxLen)
{
    this -> maxLen = maxLen;
}
...
template <class T> bool SortedLinkedList<T>::cull(T filterObj) 
{
    ...
    {   
        ...
        {
            std::cout<<"cull last filter"<<std::endl;
            free(cur);
            headptr = nullptr;
            tailptr = nullptr;
            len = 0;
